...
void SearchServer::AddDocument(int document_id, const std::string& document, DocumentStatus status, 
    ...
    if (documents_.count(document_id) > 0) {
        throw std::invalid_argument("there is already such an id");
    }
    ...
    for (const std::string& word : words) {
        if (!IsValidWord(word)) {
            throw std::invalid_argument("invalid characters in document");
        }
        else {
...
std::vector<Document> SearchServer::FindTopDocuments(const std::string& raw_query, DocumentStatus status) const {
    auto status_1 = status;
    return FindTopDocuments(raw_query, [status_1](int document_id, DocumentStatus status, int rating) {
...
SearchServer::Query SearchServer::ParseQuery(const std::string& text) const {
    ...
    for (const std::string& word : SplitIntoWords(text)) {
        if (!IsValidWord(word)) {
            throw std::invalid_argument("invalid characters in query");
        }
